...
fn process_folder(path : &Path) -> Option<Project> {
    if let Ok(toml_content) = fs::read_to_string(path.join("Cargo.toml")) {
        ...
        let folder = path.file_name().map_or("".to_string(),|v| v.to_os_string().into_string().unwrap());
        Some(Project{ desc, sub, dep, name, folder })
    } else {
        None
    }
}
...
fn print_project(p: &Project) {
    ...
    fn print_project_1(p: &Project, crates: &[String], level: usize) {
        ...
        let (in_workspace, outside_workspace) : (Vec<_>, Vec<_>)= p.dep.iter().partition(|dep| crates.iter().any(|p| &p==dep));
        let (in_workspace, outside_workspace) = (itertools::join(in_workspace,", "), itertools::join(outside_workspace,", "));
